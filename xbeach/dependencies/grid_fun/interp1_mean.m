function vi = interp1_mean(x,v,xi)
%INTERP1_MEAN  Interpolates to mean value of data points nearest to xi.
%
%   INTERP1_MEAN interpolates values v at positions x to interpolation
%   positions xi based on the nearest-neighbour principle. The mean value
%   is taken over all points x that are nearest to xi, ignoring nan's.
%   If there are no values closest to xi(n), interp1_mean returns a nan in
%   vi(n).
%
...
%
%   See also: interp1, interp1_median, griddata_average
...
%Determine mean value for points closest to xi
for n=1:length(xi)
    mask= x>=xb(n) & x<xb(n+1);
    vi(n)=mean(v(mask),'omitnan');
%     if nargout>1
